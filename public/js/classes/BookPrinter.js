...
export default class BookPrinter extends THREE.EventDispatcher {
  ...
  print() {
    ...
    if(this.type==='pdf') {
      ...
      if(this.useIFrame()) {
        ...
        if(!this.frame) {
          this.frame = $('<iframe src="'+this.pdfSrc+'" style="display: none;"></iframe>').appendTo(document.body);
        }
  ...
  renderHtml(head, body, src) {
    return new Promise((resolve, reject)=> {
      $.ajax({url: src, dataType: 'text'}).done((html)=> {
