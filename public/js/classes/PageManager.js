...
export default class PageManager {
  ...
  constructor(visual, book, p) {
    ...
    this.tmpMaterialMesh = new THREE.Mesh(new THREE.PlaneGeometry(1e-3, 1e-3), this.tmpMaterial)
  ...
  getLayers(n, clb) {
    const cssLayersLoader = this.p.cssLayersLoader? this.p.cssLayersLoader: (n, f)=> f([]);
    cssLayersLoader(n, (layers)=> {
      const object = this.pageCache.get(n);
      if(object) {
        (object.wrapper.getCSSLayer && this.p.highlightLinks? object.wrapper.getCSSLayer(): Promise.resolve()).
        then((l)=> {
          if(l || layers[0]) {
            l = l || {};
            layers[0] = layers[0] || {};
          ...
          if(object.state!=='active') {
            object.pendings.push({clb, args: [layers]});
          }
          else {
            clb(layers);
          }
  ...
  matchImageToPage(image, pageN) {
    const p = this.isCover(pageN)? this.p.cover: this.p.page, bookK = p.width/p.height;
    let bookW = image.width, bookH = Math.ceil(image.width/bookK), imageW = bookW, imageH = bookH;
        ...
        o.wrapper.onChange = (image, canceled)=> {
          if(canceled) {
            ...
            if(o.texture) {
              ...
              o.texture.image = this.imageToPageTexture(o, n, image);
  ...
  calcResolution(o) {
    let res;
    if(this.p.autoResolution.enabled) {
      const k = this.p.autoResolution.coefficient*this.p.zoom*Math.sqrt(this.visual.wnd.devicePixelRatio || 1)*Math.min((this.isSinglePage(o) || this.book.isHalfBookMode()? 1: 0.5)*this.visual.width()/o.widthTexels, this.visual.height()/o.heightTexels);
      res = {
  ...
  getFreeCanvas() {
    const c = this.canvases[this.nextCanvas];
        ...
        this.pageCache.forEach((ent)=> {
          ...
          if(o.simulate) {
            if(toObject===o && o.match && (o.match.width!==o.match.pageWidth || o.match.height!==o.match.pageHeight)) {
              const x = uv.x*o.match.pageWidth, y = (1-uv.y)*o.match.pageHeight;
